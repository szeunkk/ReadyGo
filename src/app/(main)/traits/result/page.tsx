...
import { AnimalType, getAnimalTypeMeta, getAnimalCompatibility } from '@/commons/constants/animal';
...
export default async function Page() {
  ...
  if (!result) {
    return (
      <div style={{ 
        ...
        <p style={{ fontSize: '18px', fontWeight: 600 }}>성향 테스트 결과를 찾을 수 없습니다.</p>
        <p style={{ fontSize: '14px', color: '#666' }}>먼저 성향 테스트를 완료해주세요.</p>
      </div>
    );
  }
  ...
  const radarData: { trait: 'cooperation' | 'exploration' | 'strategy' | 'leadership' | 'social'; value: number }[] = [
  ...
  const matchTypes = compatibility?.bestMatches.map((type) => getAnimalTypeMeta(type).label) || [];
