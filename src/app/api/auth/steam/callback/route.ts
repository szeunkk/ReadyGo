...
export const POST = async (request: NextRequest) => {
  try {
    ...
    // /api/auth/session과 동일한 방식으로 세션 확인
    ...
    if (error || !user) {
      ...
      if (refreshToken) {
        ...
        if (!refreshError && sessionData.session) {
          ...
          if (!refreshedError && refreshedUser) {
            // 갱신 성공: 사용자 정보 사용하여 계속 진행
            const currentUserId = refreshedUser.id;
            const workingSupabase = refreshedSupabase;
            ...
            const { data: existingLogs, error: checkLogError } =
              await workingSupabase
            ...
            if (
              ...
              // eslint-disable-next-line no-console
              console.error(
                'Error checking existing sync logs:',
                checkLogError
              );
            }
      ...
      // 토큰 갱신 실패 또는 refresh token이 없으면 에러 반환
      return NextResponse.json(
        {
          success: false,
          error: '로그인 세션이 없습니다. 다시 로그인해주세요.',
