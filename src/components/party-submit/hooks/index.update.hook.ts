...
import type { PartySubmitFormData } from './index.submit.hook';
...
interface UsePartyUpdateOptions {
  partyId: number;
  onSuccess?: () => void;
  onError?: (error: string) => void;
}

export const usePartyUpdate = (options: UsePartyUpdateOptions) => {
  ...
  const onSubmit = handleSubmit(async (data: PartySubmitFormData) => {
    ...
    try {
      ...
      // API Route를 통해 파티 수정
      const response = await fetch(`/api/party/${options.partyId}`, {
        method: 'PATCH',
        headers: {
      ...
      if (!response.ok) {
        ...
        throw new Error(errorData.error || '파티 수정 중 오류가 발생했습니다.');
      }
      ...
      if (!updateData) {
        throw new Error('수정된 파티 데이터를 가져올 수 없습니다.');
      }
      ...
      openModal({
        variant: 'dual',
        title: '수정 완료',
        description: '파티가 성공적으로 수정되었습니다.',
        onConfirm: () => {
          // 모든 모달 닫기
          closeAllModals();
          // 상세 페이지로 이동하여 리프레시
          // router.push()만으로는 같은 URL일 때 리프레시가 안 되므로
          // window.location.href를 사용하여 강제 리프레시
          window.location.href = getPartyDetailUrl(options.partyId);
          options?.onSuccess?.();
        },
      });
    } catch (error: unknown) {
      ...
      openModal({
        variant: 'dual',
        title: '수정 실패',
        description: `파티 수정 중 오류가 발생했습니다. ${errorMessage}`,
        onConfirm: () => {
          // 모달만 닫고 페이지 이동하지 않음
        },
      });
