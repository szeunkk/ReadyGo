...
test.describe('카카오 OAuth 로그인', () => {
  ...
  test.skip('성공 시나리오: 카카오 최초 로그인 시 초기 데이터 생성 및 회원가입 성공 페이지 이동', async ({
    page,
    context,
  }) => {
    // 실제 Supabase Auth 사용 (모킹 금지)
    // 주의: 이 테스트는 실제 카카오 인증이 필요합니다.
    // 카카오 로그인 버튼을 클릭하면 카카오 인증 페이지로 리다이렉트되며,
    // 인증 완료 후 /login?code=... 형태로 리다이렉트됩니다.
    ...
    // 카카오 인증 페이지로 리다이렉트되었는지 확인
    // 실제로는 현재 페이지가 카카오 인증 페이지로 리다이렉트됩니다.
    // 또는 새 창이 열릴 수 있습니다.

    // 실제 카카오 인증을 완료한 후 로그인 페이지로 리다이렉트될 때까지 대기
    // OAuth 콜백은 /login?code=... 형태로 리다이렉트됩니다.
    // 주의: 실제 카카오 인증을 완료해야 합니다.

    // 로그인 페이지로 리다이렉트될 때까지 대기 (OAuth 콜백 처리)
    // 실제로는 카카오 인증 후 자동으로 리다이렉트됩니다.
    await page.waitForURL(/.*\/login/, { timeout: 30000 }).catch(() => {
      // 리다이렉트가 안 되면 수동으로 로그인 페이지로 이동
      // (실제 카카오 인증을 완료한 후)
    });
    ...
    if (result === 'success') {
      ...
      throw new Error(
        `카카오 로그인 실패: ${errorMessage}. OAuth 설정을 확인하거나, 실제 카카오 계정으로 로그인을 완료했는지 확인하세요.`
      );
    } else {
      throw new Error(
        '카카오 로그인 후 회원가입 성공 페이지로 이동하지 않았고, 에러 모달도 표시되지 않았습니다. 네트워크 지연 또는 OAuth 설정 문제일 수 있습니다. 실제 카카오 인증을 완료했는지 확인하세요.'
      );
    }
  });
  ...
  test.skip('실패 시나리오 1: user_profiles insert 실패', async ({ page }) => {
    ...
    // OAuth 콜백 시뮬레이션: 로그인 페이지로 리다이렉트
    // 실제로는 카카오 인증 후 /login?code=... 형태로 리다이렉트됩니다.
    await page.goto('/login?code=test-oauth-code', {
      waitUntil: 'domcontentloaded',
    });

    // 가입실패모달 노출 확인
    const errorModalTitle = page.locator('text=가입실패');
    ...
    // 페이지 이동 없음 확인 (에러 발생 시 로그인 페이지에 머물러야 함)
  ...
  test.skip('기존 유저 시나리오: 카카오 로그인 시 홈 페이지로 이동', async ({
    page,
    context,
  }) => {
    // 실제 Supabase Auth 사용 (모킹 금지)
    // 주의: 이 테스트는 실제 카카오 인증이 필요하며,
    // 이미 user_profiles 레코드가 존재하는 계정으로 로그인해야 합니다.
    ...
    // 홈 페이지로 이동 확인
    const homePageSelector = '[data-testid="home-page"]';
    const errorModalSelector = 'text=가입실패';

    // 홈 페이지 또는 에러 모달 중 하나가 나타날 때까지 대기
    const result = await Promise.race([
      page
        .waitForSelector(homePageSelector, { timeout: 10000 })
        .then(() => 'success'),
      page.waitForURL(/.*\/home/, { timeout: 10000 }).then(() => 'success'),
      page
        .waitForSelector(errorModalSelector, { timeout: 10000 })
        .then(() => 'error')
        .catch(() => null),
    ...
    if (result === 'success') {
      ...
      throw new Error(
        '카카오 로그인 후 홈 페이지로 이동하지 않았고, 에러 모달도 표시되지 않았습니다. 네트워크 지연 또는 OAuth 설정 문제일 수 있습니다.'
      );
    }
  });
});
