...
test.describe('Traits Survey E2E: 설문 기본 흐름', () => {
  ...
  test('초기 상태: 첫 번째 질문이 표시된다', async ({ page }) => {
    const stepCounter = await page.textContent(
      '[data-testid="survey-step-counter"]'
    );
  ...
  test('답변 선택: 답변 버튼 클릭 시 자동으로 다음 질문으로 이동', async ({
    page,
  }) => {
    const initialStep = await page.textContent(
      '[data-testid="survey-step-counter"]'
    );
    ...
    const nextStep = await page.textContent(
      '[data-testid="survey-step-counter"]'
    );
  ...
  test('이전 버튼 비활성화: 첫 번째 질문에서는 이전 버튼이 비활성화', async ({
    page,
  }) => {
  ...
  test('진행률: 답변 진행에 따라 진행률이 증가', async ({ page }) => {
    ...
    const newProgress = await page.textContent(
      '[data-testid="survey-progress"]'
    );
  ...
  test('답변 유지: 이전/다음 이동 시 선택한 답변이 유지됨', async ({
    page,
  }) => {
    // 5점 답변 선택
    await page.click('[data-testid="answer-button-5"]');
...
test.describe('Traits Survey E2E: 전체 설문 완료', () => {
  test('전체 시나리오: 모든 질문에 답변하고 결과 페이지 도달', async ({
    page,
  }) => {
    await page.goto('/traits');
    await page.waitForSelector('[data-testid="survey-step-counter"]');
