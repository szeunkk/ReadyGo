...
export const getMyProfileService = async (
  ...
  if (traitsResult.error) {
    throw new Error(
      `Failed to fetch user traits: ${traitsResult.error.message}`
    );
  }
  ...
  if (schedulesResult.error) {
    throw new Error(
      `Failed to fetch user schedules: ${schedulesResult.error.message}`
    );
  }
  ...
  if (hasTraits && !hasSchedules) {
    throw new Error(
      'Data inconsistency: traits exist but schedules are missing'
    );
  }

  if (!hasTraits && hasSchedules) {
    throw new Error(
      'Data inconsistency: schedules exist but traits are missing'
    );
  }
