...
export const updatePartyPostService = async (
  ...
  const trimmedInput: UpdatePartyPostInput = {
    ...input,
    ...(input.game_title !== undefined && { game_title: input.game_title.trim() }),
    ...(input.party_title !== undefined && { party_title: input.party_title.trim() }),
    ...(input.description !== undefined && { description: input.description.trim() }),
    ...(input.control_level !== undefined && { control_level: input.control_level.trim() }),
    ...(input.difficulty !== undefined && { difficulty: input.difficulty.trim() }),
  };

  try {
    const post = await partyPostsRepository.updatePartyPost(postId, trimmedInput);
